...
modules by adding a `*.txt` file to the top-level `addons_paths/` directory. The
...
```
addons_paths/               ← one .txt file per project
  my_project.txt
  another_project.txt
...
### `addons_paths/<project>.txt` format

Each line is a path **relative to the project root** (`18.0-doodba/`). Comments (`#`)
and blank lines are ignored.

```
# addons_paths/my_project.txt
...
| `addons_paths/`               | One `.txt` file per project; each lists module paths relative to project root |
