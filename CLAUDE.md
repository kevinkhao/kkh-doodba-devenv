...
| `container_configs/`          | One `.txt` file per project; each lists module paths relative to project root. Entries are directory-level: every immediate subdirectory of a listed path gets symlinked, so listing a shared repo (e.g. `360_community`) to depend on one or two of its modules symlinks the whole repo. |
